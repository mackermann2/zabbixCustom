...
#
# Last Updated: 15-10-2019 (Matthieu ACKERMANN)
#
# Revision History:
#
# Version 4.10a
#  - Add the ability to connect via a proxy server (after openssl 1.1.1 compilation)
#
# Version 4.10
...
# Proxy server to use
PROXY_SERVER="fr.proxy.saint-gobain.com:8080"
...
check_server_status() {
	...
	TLSFLAG=(s_client -crlf -connect "${1}":"${2}" -proxy "$PROXY_SERVER")
    else
        TLSFLAG=(s_client -crlf -connect "${1}":"${2}" -proxy "$PROXY_SERVER" -servername "${1}")
    fi

    echo "" | "${OPENSSL}" "${TLSFLAG[@]}" 2> "${ERROR_TMP}" 1> "${CERT_TMP}"
#    echo "${OPENSSL}" "${TLSFLAG[@]}" 
